...
if(isset($_POST['a'])) {
	switch(strip_tags($_POST['a'])) {
		...
		//	echo file_get_contents('../../data/'.$u.'/support.json');
			...
			$a = array(); foreach($c['list'] as $r) $a[] = $r;
			$c['list'] = $a; // Obj to Array
			$out = json_encode($c);
			echo $out;
		}
		else echo false;
		...
		if(file_exists('../../data/'.$u.'/support/support'.$i.'.json') && file_exists('../../data/_sdata-'.$sdata.'/users.json')) {
			...
			$d = array(); foreach($c['list'] as $r) $d[] = $r;
			$c['list'] = $d; // Obj to Array
			foreach($c['list'] as $r) {
				if($r['i']==$i) {
					$a['r'] = $r['r']; // resolve
					break;
		...
		if($i!='' && file_exists('../../data/'.$u.'/support/support'.$i.'.json')) { // add in a topic
			...
			$out = json_encode($b, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
			// LIST
			...
			$out1 = json_encode($a, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
			//
			...
			$a['mail'] = ',' . ($e?$m.',':'');
			$out = json_encode($a, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
...
function bb2html($f) {
	...
	foreach($unicode as $k=>$v) str_replace($k, $v, $f);
	//
	$f = preg_replace('/\[.*?\]/', '', $f); // Remove others [...]
	return $f;
}
?>
